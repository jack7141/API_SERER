...
def detect_center_point(grab_image):
    ...
    # FIXME: 라인오차 잡아야함
    lines = cv.HoughLinesP(edges,1,np.pi/180,20,minLineLength=0,maxLineGap=300)
    
    if lines is None:
        return -1,-1,-1,-1

    top_line_p1_x1, top_line_p1_y1, top_line_p1_x2, top_line_p1_y2, bottom_line_p1_x1, bottom_line_p1_y1, bottom_line_p1_x2, bottom_line_p1_y2  = select_houghlines(lines, cX, cY)
...
def select_houghlines(lines,cX,cY):
    ...
    return top_line_p1_x1, top_line_p1_y1, top_line_p1_x2, top_line_p1_y2, bottom_line_p1_x1, bottom_line_p1_y1, bottom_line_p1_x2, bottom_line_p1_y2 
                 

def main(img2, actual_external_diameter):
    original_img = img2
    cX = 0
    cY = 0
    ...
    try:
        while True:    
            if int(abs(upper_angle)) < 97 and int(abs(upper_angle)) >= 88 and int(abs(bottom_angle)) < 97 and int(abs(bottom_angle)) >= 88:
                depth = actual_external_diameter * focal_lenth / distance
                return int(depth)          
            else:
                count += 1
                for_rotate_img = original_img.copy()
                M = cv.getRotationMatrix2D((cX,cY), -(180-int(rotate_angle)), 1)
                rotate = cv.warpAffine(for_rotate_img, M, (w, h))
                rotate_angle, distance, upper_angle, bottom_angle = detect_center_point(rotate) 
                if rotate_angle == -1 or count == 4:
                    print("There is Doesnt exist intersection")
                    break
